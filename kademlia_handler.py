...
PONG_RESPONSE = int(os.getenv("PONG_RESPONSE"))
...
class KademliaHandler:
        ...
        try:
            match message_type:
                ...
                case Message.PING:
                    ...
                    +-----------------+----------------+---------------+---------------+
                    """
                    return_status = await self.handle_ping(reader, writer, body)
    ...
    async def handle_ping(self, reader, writer, body: bytes):
        """
        ...
        response = struct.pack(">HH", message_size, PONG)
